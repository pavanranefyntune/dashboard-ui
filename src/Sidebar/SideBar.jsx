 ...
 const handleDrop = (e, droppedItemId) => {
  ...
  return (
    <div className="flex flex-col p-2">
      <Link to="/">
      <p className={` mb-8 lg:text-2xl md:text-xl text-md font-bold p-4 ${darkMode && "text-white"}`}>Niond</p>
      </Link>
    <div className={`flex flex-col justify-between gap-3 lg:text-sm text-[0.5rem] md:text-[0.7rem] font-bold ${darkMode && "text-white"}`}>
      ...
      { sideBarData.map((sData) => (
        ...
        >
          <p className={`lg:p-[8px] rounded-md ${darkMode ? "hover:bg-[#90ad40]" :"hover:bg-[#CDF463]"} cursor-pointer gap-2 flex items-center`}>{<sData.icon />}{sData.name}</p>
           </Link> 
     )) }   
